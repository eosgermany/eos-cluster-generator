...
cp ../config.ini_eosgermany_2 ./tmp2/

docker run -ti  \
    --network=host \
    -v `pwd`/tmp2:/data -v `pwd`/tmp2:/conf \
    heipacker2016/eos:v1.0.1-unstake-5-mins-eos \
    /opt/eosio/bin/nodeos \
    --genesis-json /conf/genesis.json \
    --data-dir /data \
    --config-dir /conf \
    --config config.ini_eosgermany_2 \
    --http-server-address=0.0.0.0:3002 \
    --p2p-listen-endpoint=0.0.0.0:3003 \
